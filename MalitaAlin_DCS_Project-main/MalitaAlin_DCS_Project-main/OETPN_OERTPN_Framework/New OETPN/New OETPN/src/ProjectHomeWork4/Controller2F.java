...
public class Controller2F {
	...
	public static void main (String []args) {
		...
		pn.PetriNetName = "Controller2F";
		pn.SetName("Controller2F");
		pn.NetworkPort = 1094;
		...
		ini.SetName("ini");
		ini.SetValue("red");
		pn.ConstantPlaceList.add(ini); //added to constant place list -> 
		...
		p1.SetName("r1r2"); //r1 -> Traffic Light TL1 & TL3 | r2 ->TL2 & TL$
		p1.SetValue("signal");
		pn.PlaceList.add(p1);
		...
		p6.SetName("OP1");
		p6.Value = new TransferOperation("localhost", "1091" , "P_TL1");
		pn.PlaceList.add(p6);
		
		DataTransfer p7 = new DataTransfer();
		p7.SetName("OP2");
		p7.Value = new TransferOperation("localhost", "1091" , "P_TL2");
		pn.PlaceList.add(p7);
		
		DataTransfer p8 = new DataTransfer();
		p8.SetName("OP3");
		p8.Value = new TransferOperation("localhost", "1091" , "P_TL3");
		pn.PlaceList.add(p8);
		
		DataTransfer p9 = new DataTransfer();
		p9.SetName("OP4");
		p9.Value = new TransferOperation("localhost", "1091" , "P_TL4");
		pn.PlaceList.add(p9);
		...
		grdiniT.Activations.add(new Activation(iniT, "", TransitionOperation.MakeNull, "ini")); //because ini is part of constant place list
		...
		pn.Delay = 1800;
		// pn.Start();
        //run Intersection then Controller2F, controller sends signal to traffic lights automatically (color change (token) every 5 seconds) 
